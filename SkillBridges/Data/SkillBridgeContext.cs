...
namespace SkillBridges.Data
{
    public class SkillBridgeContext:DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<TaskApplication>().HasOne(ta => ta.ProfessionalProfile).WithMany(t => t.TaskApplications).HasForeignKey(p => p.ProfessionalProfileId).OnDelete(DeleteBehavior.Cascade);

            modelBuilder.Entity<ProfessionalProfile>().HasMany(t => t.Tasks).WithOne(p => p.ProfessionalProfile).HasForeignKey(p => p.ProfessionalProfileId).OnDelete(DeleteBehavior.SetNull);
            modelBuilder.Entity<ClientProfile>().HasMany(c=>c.TaskApplications).WithOne(c=>c.ClientProfile).HasForeignKey(p=>p.ClientProfileId).OnDelete(DeleteBehavior.Cascade);
            ...
            modelBuilder.Entity<ProfessionalProfile>().HasMany(t => t.TaskApplications).WithOne(t => t.ProfessionalProfile).HasForeignKey(p => p.ProfessionalProfileId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<Rating>().HasOne(t => t.Task).WithOne().HasForeignKey<Rating>(t => t.TaskId).OnDelete(DeleteBehavior.SetNull);
            modelBuilder.Entity<Rating>().HasOne(t => t.ClientProfile).WithMany().HasForeignKey(t => t.ClientProfileId).OnDelete(DeleteBehavior.Cascade);
            ...
            modelBuilder.Entity<Payment>().HasOne(p => p.Task).WithMany().HasForeignKey(p => p.TaskId).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<Payment>().HasOne(p => p.ClientProfile).WithMany().HasForeignKey(p => p.ClientProfileId).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<Payment>().HasOne(p => p.ProfessionalProfile).WithMany().HasForeignKey(p => p.ProfessionalProfileId).OnDelete(DeleteBehavior.Restrict);
