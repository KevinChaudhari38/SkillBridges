...
namespace SkillBridges.Models
{
    public class TaskApplicationRepository:ITaskApplicationRepository
    {
        ...
        {
            return _context.TaskApplications.Include(t=>t.Task).Include(ta=>ta.ProfessionalProfile).ThenInclude(ta=>ta.User).Include(ta=>ta.ClientProfile).Where(t=>t.TaskId==taskId).ToList();
        }
        public List<TaskApplication> GetByProfessionalId(string professionalId)
        {
            return _context.TaskApplications.Include(ta => ta.Task).Include(ta=>ta.ProfessionalProfile).ThenInclude(cp=>cp.User).Include(ta=>ta.ClientProfile).Where(ta => ta.ProfessionalProfileId == professionalId).ToList();
        }
